...
namespace mfl::parser
{
    ...
    {
        return (std::find_if(font_commands.begin(), font_commands.end(), [&](const auto c) { return c.first == name; })
                != font_commands.end());
    }
    ...
    std::vector<noad> create_font_group(parser_state& state)
    {
        ...
        const auto it = std::find_if(font_commands.begin(), font_commands.end(), [&](const auto c) { return c.first == name; });
