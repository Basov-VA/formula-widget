...
namespace mfl::parser
{
    namespace
    {
        ...
        {
            const auto it = std::find_if(symbols.begin(), symbols.end(), [&](const auto entry) { return name == entry.first; });
        ...
        {
            const auto it = std::find_if(mappings.begin(), mappings.end(), [&](const auto& e) { return e.first == code; });
