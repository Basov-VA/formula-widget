...
namespace mfl
{
    namespace
    {
        ...
        // Зазор между буквой и меткой акцента. Стрелочные акценты (\vec и
        // над-стрелки, коды 0x20d6..0x20e1) нарисованы в STIX2 низко в своём боксе
        // и налезают на короткие буквы — им нужен заметно больший зазор, чем
        // «крышке»/«черте»/«тильде».
        dist_t accent_gap(const settings s, const code_point cc)
        {
            const bool is_arrow = (cc >= 0x20d6 && cc <= 0x20e1);
            return is_arrow ? rule_thickness(s) + x_height(s) * 3 / 2
                            : rule_thickness(s) + x_height(s);
        }

        hlist accent_hlist(const dist_t shift, const dist_t gap_size,
                           box&& content_box, const glyph& accent_glyph)
        {
            auto accent_box = make_hbox(make_hlist(accent_glyph));
            accent_box.shift = shift;
            const auto width = content_box.dims.width;
            auto gap = kern{.size = gap_size};
        ...
        hlist accent_over_empty_box(const settings s, const accent& a)
        {
            ...
            auto empty_box = box{.dims = {.width = box_size, .height = box_size}};
            empty_box.annotation = formula_node_type::group;
            return accent_hlist(box_size, accent_gap(s, a.char_code), std::move(empty_box),
        ...
        hlist accent_over_math_char(const settings s, const accent& a)
        {
            ...
            auto content_box = make_hbox(make_hlist(accentee_glyph));
            content_box.annotation = formula_node_type::group;
            return accent_hlist(shift, accent_gap(s, a.char_code), std::move(content_box), accenter_glyph);
        }
        ...
        hlist accent_over_complex_content(const settings s, const cramping cramp, const accent& a)
        {
            ...
            content_box.annotation = formula_node_type::group;
            return accent_hlist(shift, accent_gap(s, a.char_code), std::move(content_box), accent_glyph);
        }
    }
