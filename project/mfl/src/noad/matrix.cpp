#include "noad/matrix.hpp"

#include "node/box.hpp"
#include "node/hlist.hpp"
#include "settings.hpp"

#include <algorithm>
#include <ranges>

namespace mfl
{
    settings matrix_settings(const settings s)
    {
        if (s.style == formula_style::display) return {.style = formula_style::text, .fonts = s.fonts};
        if (s.style == formula_style::text) return {.style = formula_style::script, .fonts = s.fonts};
        return {.style = formula_style::script_script, .fonts = s.fonts};
    }

    hlist matrix_to_hlist(const settings s, const cramping, const matrix& m)
    {
        const auto cell_settings = matrix_settings(s);
        auto cell_box_rows = std::vector<std::vector<box>>{};
        for (const auto& row : m.rows)
        {
            auto cell_row = std::vector<box>{};
            for (const auto& cell : row)
            {
                auto cell_box = clean_box(cell_settings, cramping::on, cell);
                cell_box.annotation = formula_node_type::matrix_cell;
                cell_row.push_back(std::move(cell_box));
            }

            cell_box_rows.push_back(cell_row);
        }

        const auto num_rows = cell_box_rows.size();
        if (num_rows == 0) return {};

        const auto num_cols = std::ranges::max(cell_box_rows | std::views::transform(&std::vector<box>::size));

        auto col_widths = std::vector(num_cols, dist_t{0});
        for (const auto& row : cell_box_rows)
        {
            for (std::size_t index = 0; index < row.size(); ++index)
            {
                col_widths[index] = std::max(col_widths[index], row[index].dims.width);
            }
        }


        auto row_hlists = std::vector<hlist>(num_rows);
        for (std::size_t row_index = 0; row_index < cell_box_rows.size(); ++row_index)
        {
            auto&& row = cell_box_rows[row_index];
            const auto i = static_cast<std::size_t>(row_index);
            auto& row_hlist = row_hlists[i];

            for (std::size_t col_index = 0; col_index < row.size(); ++col_index)
            {
                auto&& cell_box = row[col_index];
                const auto j = static_cast<std::size_t>(col_index);
                row_hlist.nodes.emplace_back(rebox(col_widths[j], std::move(cell_box)));

                if (j + 1 < num_cols) row_hlist.nodes.push_back(glue_spec{.size = quad(s)});
            }
        }

        const auto width = hlist_width(row_hlists.front());
        auto stacked_rows = vlist{};
        auto top_row_box = make_hbox(std::move(row_hlists.front()));
        top_row_box.annotation = formula_node_type::group;  // or appropriate type for matrix row
        auto prev_row_depth = top_row_box.dims.depth;
        for (auto&& row_hlist : row_hlists | std::views::drop(1))
        {
            auto row_box = make_hbox(std::move(row_hlist));
            row_box.annotation = formula_node_type::group;  // or appropriate type for matrix row
            const auto glue_size = std::max(x_height(s), x_height(s) * 3 - prev_row_depth - row_box.dims.height);
            prev_row_depth = row_box.dims.depth;
            stacked_rows.nodes.push_back(glue_spec{.size = glue_size});
            stacked_rows.nodes.push_back(std::move(row_box));
        }

        auto result_box = center_on_axis(s, make_down_vbox(width, std::move(top_row_box), std::move(stacked_rows)));
        result_box.annotation = formula_node_type::matrix;
        return make_hlist(std::move(result_box));
    }
}
