...
namespace mfl
{
    ...
    namespace
    {
        ...
        // Реальная ширина клея после упаковки бокса: натуральный размер плюс
        // вклад от растяжения при совпадении порядка бесконечности с glue-set
        // бокса (как glue_set в TeX). Боксы этого движка только растягиваются
        // (rebox всегда паддит до максимальной ширины, width_diff >= 0), поэтому
        // учитывается компонента stretch. Без этого центрирующий unit-glue
        // (size = 0, stretch = fil) схлопывался бы в ноль и содержимое узких
        // числителей/индексов/ячеек прижималось бы влево.
        dist_t glue_set_width(const glue_spec& g, const glue_param& gp)
        {
            dist_t w = g.size;
            if (gp.scale != 0.0 && g.stretch.order == gp.order)
                w += static_cast<dist_t>(gp.scale * static_cast<double>(g.stretch.value));
            return w;
        }

        dist_t scaled_width(const node_variant& n, const glue_param& gp)
        {
            return std::visit(
                overload{[&](const kern& k) { return k.size; },
                         [&](const glue_spec& g) { return glue_set_width(g, gp); },
            ...
            for (std::size_t i = 0; i < nodes.size(); ++i)
            {
                ...
                if (std::holds_alternative<glyph>(node))
                {
                    ...
                    if (has_script_sibling)
                    {
                        ...
                        cur_x += dist_to_points(scaled_width(node, b.glue));
                        continue;
                    }
                }
        ...
        void layout_vbox(const box& b, const points x, const points y, layout_elements& elements, formula_node& parent)
        {
            ...
            for (const auto& node : b.nodes)
            {
                ...
                // Move down by the full vertical size of this node. Use vsize()
                // (height+depth) to match make_vbox, which sizes the box the same
                // way. depth()/height() fold a box's `shift` into the vertical
                // extent, but in a vbox `shift` is HORIZONTAL (e.g. accent skew);
                // counting it here would drop accented content below the baseline.
                cur_top -= dist_to_points(vsize(node));
            }
        }
