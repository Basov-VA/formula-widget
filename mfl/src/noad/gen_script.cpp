...
namespace mfl
{
    namespace
    {
                               ...
                               const std::optional<std::vector<noad>>& sup)
        {
            ...
            if (sup_node) {
                if (std::holds_alternative<wrapped_box>(*sup_node)) {
                    auto& sup_box = std::get<wrapped_box>(*sup_node);
                    static_cast<box&>(sup_box).annotation = formula_node_type::superscript;
                }
            }
            ...
            if (sub_node) {
                if (std::holds_alternative<wrapped_box>(*sub_node)) {
                    auto& sub_box = std::get<wrapped_box>(*sub_node);
                    static_cast<box&>(sub_box).annotation = formula_node_type::subscript;
                }
            }
            ...
            nuc_box.annotation = formula_node_type::script_nucleus;
            auto vbox = make_vbox(w, std::move(nuc_box),
                                  limit_sup_list(s, w + italic_correction, shift, std::move(sup_node)),
                                  limit_sub_list(s, w + italic_correction, -shift, std::move(sub_node)));
            vbox.annotation = formula_node_type::superscript;  // Using superscript as the main annotation
                                ...
                                const std::optional<std::vector<noad>>& sup)
        {
            ...
            if (sub_box && !sup_box) {
                sub_box->annotation = formula_node_type::subscript;
                return make_subscript_hlist(s, std::move(nucleus), std::move(*sub_box));
            }
